  ...
  return (
    <motion.div
      ...
      className={`bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl relative h-full flex flex-col ${
        featured
          ? "ring-2 ring-zaploom shadow-lg"
          : "border border-gray-100 dark:border-gray-700"
      }`}
    >
      {featured && (
        <>
          <div className="absolute top-0 left-0 right-0 h-1 bg-zaploom" />
          <motion.span
            initial={{ y: -20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.4, delay: 0.1 }}
            className="absolute -top-4 right-4 bg-zaploom text-white text-sm font-medium px-4 py-1 rounded-full shadow-lg z-10"
          >
            Best Value
          </motion.span>
        </>
      ...
      <div className="p-6 md:p-8 flex flex-col h-full">
        <div className="mb-6 pt-3">
          ...
          <p className="text-lg font-semibold text-zaploom dark:text-zaploom-light">
            {price}
          </p>
        ...
        <div className="mb-6">
          <Badge
            variant="outline"
            className="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-normal"
          >
            {ideal}
          </Badge>
        </div>
